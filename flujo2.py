...
FIG_ANCHO = float(os.getenv("FIG_ANCHO", "12.2"))
FIG_ALTO = float(os.getenv("FIG_ALTO", "12.4"))
FRANJA_ALTO = float(os.getenv("FRANJA_ALTO", "0.62"))
FRANJA_MIN = float(os.getenv("FRANJA_MIN", "1"))
PRECIO_ALTO = float(os.getenv("PRECIO_ALTO", "3.1"))
TOTAL_ALTO = float(os.getenv("TOTAL_ALTO", "1.15"))
QDELTA_ALTO = float(os.getenv("QDELTA_ALTO", "1.15"))
...
def niveles(tk, fecha, spot=None):
    ...
    for src in ("oi", "vol"):
        ...
        for k, c in (("CW","call_wall"),("PW","put_wall"),("QF","gamma_flip"),("MAGNET","gamma_magnet")):
            ...
            if v is None:
                continue
            if spot and abs(v - spot) / max(abs(spot), 1) > 0.035:
                continue
...
def procesar_df(data, ticker):
    ...
    for _, r in df.iterrows():
        if r["lado"]=="COMPRA" and r["contrato"]=="CALL": sg.append(1)
        elif r["lado"]=="COMPRA" and r["contrato"]=="PUT": sg.append(-1)
        elif r["lado"]=="VENTA" and r["contrato"]=="CALL": sg.append(-1)
        elif r["lado"]=="VENTA" and r["contrato"]=="PUT": sg.append(1)
        else: sg.append(0)
    df["signo"] = sg
...
def tape(fecha, ticker):
    ...
    return procesar_df(out, ticker).drop_duplicates(["hora","premium","size"]).sort_values("hora")
...
def grafico(grupo, df, etiqueta, fecha, niv, vol, net):
    ...
    fig, axs = plt.subplots(
        4, 1, figsize=(FIG_ANCHO, FIG_ALTO), sharex=True, facecolor=bg,
        gridspec_kw={"height_ratios": [PRECIO_ALTO, FRANJA_ALTO, TOTAL_ALTO, QDELTA_ALTO], "hspace": 0.05},
    )
    ax1, axA, axT, axQ = axs
    for ax in axs:
        ax.set_facecolor(bg)
    ...
    extras = [v for v in niv.values() if v and abs(v-last)/max(abs(last),1) <= 0.035]
    if extras:
    ...
    ax1.set_title(f"{grupo}  {etiqueta}  {fecha}   |   {regimen} {sesgo}   |   qΔ {fmt_usd(neto)}   flow {ratio:.2f}{iv}",
                  color=fg, loc="left", fontsize=12, pad=6)
    fig.text(0.01, 0.01, f"NY {datetime.now(TZ):%H:%M}   COL {datetime.now(TZ_COL):%H:%M}   print ≠ dirección ETF",
        ...
        if df is not None and not df.empty:
            for _, r in df.nlargest(3, "exposicion").iterrows():
                if r["exposicion"] >= ALERTA_USD:
                    telegram(f"{grupo} {etiqueta} {fmt_usd(r['exposicion'])} {r['lado']} {r['contrato']}")
...
def main():
    ...
    print("MODO", modo, "franja", FRANJA_ALTO, "min", FRANJA_MIN)
    resumen = []
    if modo in ("AYER","AMBOS"): procesar(ayer(), "AYER", resumen)
    if modo in ("HOY","AMBOS"): procesar(hoy, "HOY", resumen)
