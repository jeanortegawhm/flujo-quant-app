...
MIN_BURBUJA = {"SPX":8_000_000,"SPY":5_000_000,"QQQ":5_000_000,"IWM":2_000_000,"IBIT":1_000_000,"GLD":2_000_000}

MIN_PREMIUM = int(os.getenv("MIN_PREMIUM", "500000"))
...
FIG_ALTO = float(os.getenv("FIG_ALTO", "11.6"))
FRANJA_ALTO = float(os.getenv("FRANJA_ALTO", "0.28"))
FRANJA_MIN = float(os.getenv("FRANJA_MIN", "3"))
PRECIO_ALTO = float(os.getenv("PRECIO_ALTO", "3.4"))
...
def grafico(grupo, df, etiqueta, fecha, niv, vol, net, extra):
    ...
    umbral = UMBRAL_BURBUJA if UMBRAL_BURBUJA > 0 else MIN_BURBUJA.get(grupo, 3_000_000)
    ...
    ax1.fill_between(px.index, px["Close"], px["Close"].min(), color="#6ea8ff", alpha=0.08)
    ...
    if not grandes.empty:
        axT.bar(grandes["hora"], grandes["premium"] / 1e6, width=0.0032, color=gold, alpha=0.92)
        for _, r in grandes.nlargest(4, "premium").iterrows():
    ...
    ax1.set_title(
        ...
        f"prima net {fmt_usd(neto)}  30m {fmt_usd(qd30)}  flow {ratio:.2f}{iv}",
        color=fg, loc="left", fontsize=10, pad=6,
    )
    fig.text(0.01, 0.008,
             f"NY {datetime.now(TZ):%H:%M}  COL {datetime.now(TZ_COL):%H:%M}  "
             f"anillo = PRIMA (como Quantium)  |  print ≠ dirección ETF",
